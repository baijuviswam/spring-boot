package com.lab.aws.storage.message;
...
public class QueueListener {
	...
	public void onS3UploadEvent(S3EventNotification event) {
		...
		if (event.getRecords().size() > 0) {
			...
			Message<String> payload = MessageBuilder.withPayload("New upload happened: " + bucket + "/" + key).build();
		}

		// this.queueMessagingTemplate.convertAndSend("queueNameToNotify", payload);
		// this.notificationMessagingTemplate.convertAndSend("topicNameToNotify",
		// payload);
	}
}
